...
class Reversi:
    ...
    def VerSiNodeTerminal(self,tablero, jugador):
        for y in range(n):
            for x in range(n):
                if self.ValidarMovimiento(tablero, x, y, jugador):
                    return False
        return True
    ...
    def EvalBoard(board, player): #comentar
        tot = 0
        for y in range(n):
            for x in range(n):
                if board[y][x] == player:
    ...
    def Minimax(self,tablero, jugador, dificultad, VerPlayerAMaximizar): #lo que hace esta funcion es buscar maximizar la puntacion del jugador 2 y minimizar la puntuacion del jugador 1 
        if dificultad == 0 or self.VersiNodoTerminal(tablero, jugador):
            return self.EvalBoard(tablero, jugador)  #Devolver el valor heur
        if VerPlayerAMaximizar: #maximizar al jugador 
            v = maxEvalBoard
            for y in range(n):
                for x in range(n): #revisar todo nodo
                    if self.ValidarMovimiento(tablero, x, y, jugador):
                        valor = self.Minimax(self.tableroTem, jugador, dificultad - 1, False) #recursividad
                        bestValue = max(bestValue, valor)
        else: #minimizar al jugador 
            v = minEvalBoard
            for y in range(n):
                for x in range(n):
                    if (self.ValidarMovimiento(tablero, x, y, jugador)):

                        valor = self.Minimax(self.tableroTem, jugador, dificultad - 1, True)
                        bestValue = min(bestValue, valor)
        return bestValue
